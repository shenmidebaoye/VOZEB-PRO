...
- [桌面] 布局视口高度扣除自定义顶栏，窗口缩放后主工作区与弹层不再被裁切。
...
- [设置] AuthSettings 与设置持久化/归一化/PATCH 已去掉注册、SMTP、积分倍率、成本保护、套餐权益和 `payment_config`；数据维护只保留到期生成任务和临时媒体。
- [生成] 系统代理不再扣积分；本机实例自动绑定本地 owner，无需账号体系。
- [安装] 安装向导两步完成环境初始化，不再创建管理员账号。
- [瘦身] 删除支付/订单/优惠券/邀请/积分钱包服务与仓储、商业 schema 模块及客户端计费 API；CDK 兑换返回已移除。
- [安装] 数据库初始化不再要求一次性安装令牌。
- [开发] 本地 `pnpm run dev` 改回 Next.js 默认 Turbopack，避免 webpack 按路由现编导致点击卡住。
- [生成] OpenAI 兼容图片网关用 `code: 200` 包装成功结果时，不再误判失败并自动重提上游任务。
- [开发] `pnpm start` / `start:local` 未设置时默认 `VOZEB_PRO_DATABASE_PROVIDER=file`。
- [工程] 去掉 Vitest 单元测试与 Playwright e2e，发布门禁只保留类型检查、lint、格式和生产构建。
- [瘦身] 删除积分/CDK/公告/验证码 stub 与计费倍率类型；系统代理只按能力识别创建请求，不再读取积分倍率。
- [瘦身] `repository-types` 去掉支付/积分/社区等商业类型；去掉会话清理开关与无操作 auth 维护；管理员权限改为本机 owner 可用校验。
- [身份] 客户端 401 不再跳转已删除的 `/login`，改为回首页并清除瞬时状态；删除无引用的登录环境提醒类型。
- [部署] 移除 Docker / Compose / Render / 宝塔镜像部署与相关工作流；安装向导与文档改为本机 `pnpm start`/`dev` 与可选 PostgreSQL。

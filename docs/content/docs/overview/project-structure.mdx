...
```text
VOZEB-PRO/
|- .github/workflows/       质量检查工作流
|- desktop/                 Tauri 桌面壳（启动 splash + 拉起本机 sidecar）
|  |- splash/               启动等待页
|  `- src-tauri/            Rust 壳、图标与打包配置
|- docs/                    Fumadocs 文档站
|  |- content/docs/         产品、后端与进度文档
|  |- public/community/     QQ 群等项目社区素材
|  |- public/screenshots/   文档使用的脱敏页面截图
|  `- src/                  文档站页面、搜索和导航
|- web/                     VOZEB PRO 主应用
|  |- public/               Logo、图标等静态资源
|  |- scripts/              本机启动与运维脚本（含 start-desktop.mjs）
|  `- src/
|     |- app/               页面、布局和 API Route Handler
|     |- components/        跨页面组件与设置界面
|     |- hooks/             跨页面交互与生成流程 Hook
|     |- lib/               领域契约、认证和服务端业务
|     |- services/api/      浏览器访问本站 API 的客户端
|     |- stores/            Zustand 全局瞬时状态
|     `- styles/            跨页面样式
|- .env.example             本机环境变量模板，不包含真实密钥
...
| `web/scripts/start-standalone.mjs`                  | standalone 生产进程启动和运行目录准备                       |
| `web/scripts/release-check.mjs`                     | 本机发布前类型检查、lint、格式与构建                        |
| `.github/workflows/quality.yml`                     | Web/Docs 依赖安装、类型检查、格式检查和生产构建             |

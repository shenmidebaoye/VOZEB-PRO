...
export async function requireManagedMediaInputOwner(value: string, actor: { id: string }, scope: "reference" | "generation") {
    ...
    if (!registration || registration.scope !== scope || isLocalMediaRegistrationExpired(registration) || registration.ownerUserId !== actor.id) {
